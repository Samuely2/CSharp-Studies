...
var produtoEletronicos = listaProdutos.Where(p => p.Categoria == "Eletrônicos"); //Filtrando usando apenas uma condição
...
Console.WriteLine();
foreach (var item in produtoEstoqueMinimo)
{
    Console.WriteLine($"{item.Nome} \t{item.Preco:C2}");
}

// Ordenado por mais de um critério
Console.WriteLine("\nProdutos ordenados por categoria e nome");
var produtosOrdenadosPorCategoriaENome = listaProdutos.OrderBy(p => p.Categoria)
                                                      .ThenBy(p => p.Nome);
